...
checkGM = ()->
  ...
  winner =
    switch
      ...
      when 0 == c.WOLF
        'HUMAN'
      when TABULA && c.HUMAN <= c.WOLF
        'WOLF'
      when MISTERY && 0 == c.VILLAGER
        'WOLF'
      else
        null
...
startGM = ->
  deployment()
  for o, idx in Query.stats.shuffle.list
    switch o.able_id
      when setfriends
        ToDo
      when 'rob'
        history discards.join("、")
      when 'stigma'
        o.label = "#{codes[idx]}の#{o.role.label}"
...
updateGM = ->
  ...
  # 突然死処理
  suddendeath = Query.potofs.where (o)-> o.live.role_id == 'live' && o.stats.SSAY.count == 0
  for o in suddendeath.list
    now_dead 'suddendead'
  ...
  for o in Query.stats.where (o)-> o.can( turn ) && ! o.potof_id
    potof = _.sample to[o.able.for].list
    o.potof_id = potof.id
...
module.exports = { startGM, deployGM, executionGM, checkGM, updateGM }
