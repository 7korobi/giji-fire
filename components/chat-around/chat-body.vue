...
.chat(:id="id" v-on="markup_event('click')")
  template(v-if="head")
    ...
    .name(v-else)
      ...
      | {{ head }}
  hr(v-if="head")
  component.text(v-bind="for_target" v-on="$listeners" v-model="v_value" v-if="v_value && $listeners['update:bans']")
  ...
  component.text(v-bind="for_target" v-on="$listeners" :value="v_value" v-if="v_value && ! $listeners['update:bans']")
